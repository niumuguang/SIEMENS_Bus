...
//	} else if (selectionPath->getTail()->isOfType(SoCylinder::getClassTypeId())) {
...
//	theApp.m_pViewer->setEventCallback();
	...
	//m_Selection->policy.setValue(SoSelection::SINGLE);
	//m_Selection-> addSelectionCallback(mySelectionCB);
	//m_Selection-> addDeselectionCallback(myDeselectionCB);
...
int COpenInventorView::hemishere( int x, int y, int d,AXIS &v)
{
	float z;
