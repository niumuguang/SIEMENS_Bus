...
{
	// 孔选择模式
	if (theApp.m_HoleStatus == true)
	{
		int SelNum = theApp.m_Selection->getNumSelected();
		SoPath * path = new SoPath(*selectionPath);
		path->ref();
		//SoSeparator * LastNode = (SoSeparator *) path->getTail();
		SoSeparator * LastNode = (SoSeparator *) path->getNodeFromTail(1);
		SbName tempName;
		tempName = LastNode->getName();
		SbString sbStr, sbStr1, sbStr2, sbStrtemp, sbNum;
		// 连接排名称
		sbStr.fromWideChar(theApp.SbName2WChar(tempName));
		//theApp.myMessageBox(sbStr);
		sbStr1 = sbStr.getSubString(0, sbStr.getLength()-3);

		sbStrtemp = sbStr.getSubString(sbStr.getLength()-10, sbStr.getLength()-2);
		sbNum = sbStr.getSubString(sbStr.getLength()-1, sbStr.getLength()-1);
		//theApp.myMessageBox(sbNum);
		if (sbStrtemp == "connectBC" || sbStrtemp == "connectAC")
		{
			if (SelNum == 3)
			{
				SoSeparator * HoleNode = (SoSeparator *) path->getNodeFromTail(1);
				/*theApp.myMessageBox(sbStr1);
				sbStrtemp = sbStr1 + "CIRCLE_MATERIAL" + sbNum;
				theApp.myMessageBox(sbStrtemp);
				SbName nametemp;nametemp = sbStrtemp;
				SoNode * HoleMaterial = HoleNode->getByName(nametemp);
				theApp.myMessageBox(HoleMaterial->getName());*/

				SoNode * HoleMaterial = HoleNode->getByName(sbStr1+"CIRCLE_MATERIAL"+sbNum);
				if (HoleMaterial->isOfType(SoMaterial::getClassTypeId()))
				{
					...
					theApp.xx = HoleMaterial->getField(nameFiled);
					theApp.xx->set(temp);
				}
				//theApp.myMessageBox(sbStr1+"CIRCLE_MATERIAL"+sbNum);
			}
			else if (SelNum == 4)
			{
				SoNode * HoleMaterial = LastNode->getByName(sbStr1+"CIRCLE_MATERIAL"+sbNum);
				if (HoleMaterial->isOfType(SoMaterial::getClassTypeId()))
				{
					SbString diffuse,temp;
					SbName nameFiled;
					...
					theApp.yy = HoleMaterial->getField(nameFiled);
					theApp.yy->set(temp);
					// 对齐算法
					SoPath * Sel1 = theApp.m_Selection->getPath(2);// 第一个选取的点
					SoNode * testSel = Sel1->getNodeFromTail(1);
					SbName NameSel = testSel->getName();
					SbString CStr, CStr1, CNum, CStrTemp;
					// 连接排名称
					CStr.fromWideChar(theApp.SbName2WChar(NameSel));
					//CStr = CStr + "P4";
					//
					//sbStr.fromWideChar(theApp.SbName2WChar(tempName));
					//theApp.myMessageBox(sbStr);
					CStr1 = CStr.getSubString(0, CStr.getLength()-3);
					CStrTemp = CStr.getSubString(CStr.getLength()-10, CStr.getLength()-2);
					CNum = CStr.getSubString(CStr.getLength()-1, CStr.getLength()-1);
					//
					CStr = CStr1 + "CIRCLE_CP"+CNum;
					
					//MatrixGet.getMatrix();
					

					/*for (int i=0; i<Sel1->getLength(); i++)
					{
					SoNode * everySel = Sel1->getNodeFromTail(i);
					theApp.myMessageBox(everySel->getName());
					if (everySel->isOfType(SoTranslation::getClassTypeId()))
					{
					theApp.myMessageBox(i);
					}
					}*/

					SoNode * testSel1Coor = testSel->getByName(CStr);
					SbString diffuse1,temp1;
					SbName nameFiled1;
					nameFiled1 = "point";
					//temp = "0 1 0";
					theApp.xy = testSel1Coor->getField(nameFiled1);
					theApp.xy->get(temp1);
					float getX, getY, getZ;
					theApp.GenFiled(temp1, getX, getY, getZ);
					///////////////////////////////////////////////////
					////  测试积累变换
					SbVec3f ppp, resppp;
					float tempX, tempY, tempZ;
					//theApp.myMessageBox(tempX);
					resppp.setValue(0,0,0);
					ppp.setValue(getX, getY, getZ);
					//ppp.setValue(0,0,0);
					SoGetMatrixAction * MatrixGet = new SoGetMatrixAction(theApp.m_pViewer->getViewportRegion());
					MatrixGet->apply(Sel1);
					SbMatrix ddd = MatrixGet->getMatrix();
					ddd.multVecMatrix(ppp, resppp);
					////

					resppp.getValue(tempX, tempY, tempZ);
					//theApp.myMessageBox(tempX);
					//theApp.myMessageBox(tempY);
					//theApp.myMessageBox(tempZ);

					//ppp.getValue(tempX, tempY, tempZ);
					//theApp.myMessageBox(tempX);
					//theApp.myMessageBox(tempY);
					//theApp.myMessageBox(tempZ);
					...
					//////////////////////////////////////////////////////////////////////////
					// 对齐算法2
					SoPath * Sel2 = theApp.m_Selection->getPath(3);// 第一个选取的点
					SoNode * testSel2 = Sel2->getNodeFromTail(1);
					SbName NameSel2 = testSel2->getName();
					//theApp.myMessageBox(NameSel2);
					SbString CStr2, CStr12, CNum2, CStrTemp2;
					// 连接排名称
					CStr2.fromWideChar(theApp.SbName2WChar(NameSel2));
					//CStr = CStr + "P4";
					//
					//sbStr.fromWideChar(theApp.SbName2WChar(tempName));
					//theApp.myMessageBox(sbStr);
					CStr12 = CStr2.getSubString(0, CStr2.getLength()-3);
					CStrTemp2 = CStr2.getSubString(CStr2.getLength()-10, CStr2.getLength()-2);
					CNum2 = CStr2.getSubString(CStr2.getLength()-1, CStr2.getLength()-1);
					//
					CStr2 = CStr12 + "CIRCLE_CP"+CNum2;
					//////////////////////////////////////////////////////////////////////////
					SoNode * testSel1Coor2 = testSel->getByName(CStr2);
					SbString diffuse12, temp12;
					SbName nameFiled12;
					nameFiled12 = "point";
					//temp = "0 1 0";
					theApp.xz = testSel1Coor2->getField(nameFiled12);
					theApp.xz->get(temp12);
					float getX2, getY2, getZ2;
					theApp.GenFiled(temp12, getX2, getY2, getZ2);

					//theApp.myMessageBox(getX2);
					//theApp.myMessageBox(getY2);
					//theApp.myMessageBox(getZ2);
					///////////////////////////////////////////////////
					////  测试积累变换
					SbVec3f ppp2, resppp2;
					float tempX2, tempY2, tempZ2;
					//theApp.myMessageBox(tempX);
					resppp2.setValue(0,0,0);
					ppp2.setValue(getX2, getY2, getZ2);
					//ppp.setValue(0,0,0);
					SoGetMatrixAction * MatrixGet2 = new SoGetMatrixAction(theApp.m_pViewer->getViewportRegion());
					MatrixGet2->apply(Sel2);
					SbMatrix ddd2 = MatrixGet2->getMatrix();
					ddd2.multVecMatrix(ppp2, resppp2);
					////
					//tempX2 = getX2; tempY2 = getY2; tempZ2 = getZ2;
					resppp2.getValue(tempX2, tempY2, tempZ2);

					//////////////////////////////////////////////////////////////////////////
					// 执行对齐
					SoPath * MovePath = theApp.m_Selection->getPath(0);// 第一个选取的点
					SoSeparator * group = (SoSeparator *) MovePath->getTail();
					if (group->isOfType(SoFaceSet::getClassTypeId()))
					{
						int testLength = MovePath->getLength();
						SoNode * testNode = MovePath->getNodeFromTail(3);
						if (testNode->isOfType(SoGroup::getClassTypeId()))
						{
							...
							{
								SbString diffuse,tempbbb;
								SbName nameFiled;
								SoField *xxtest;
								nameFiled = "translation";
								xxtest = testTranslation->getField(nameFiled);
								xxtest->get(tempbbb);
								float originalX, originalY, originalZ;
								//theApp.myMessageBox(tempbbb);
								int FirstSpacePos = tempbbb.find(" ");
								SbString dddtest1 = tempbbb.getSubString(0, FirstSpacePos);//x
								//theApp.myMessageBox(dddtest1);
								SbString dddtest2 = tempbbb.getSubString(FirstSpacePos+1,-1);
								//theApp.myMessageBox(dddtest2);
								int SecSpacePos = dddtest2.find(" ");
								//theApp.myMessageBox(SecSpacePos);
								SbString dddtest3 = dddtest2.getSubString(0, SecSpacePos);//y
								//theApp.myMessageBox(dddtest3);

								SbString dddtest4 = dddtest2.getSubString(SecSpacePos+1, -1);
								//theApp.myMessageBox(dddtest4);
								int ThirdSpacePos = dddtest4.find(" ");
								//theApp.myMessageBox(ThirdSpacePos);
								SbString dddtest5 = dddtest4.getSubString(0, ThirdSpacePos);//z
								//theApp.myMessageBox(dddtest5);
								
								originalX = dddtest1.toFloat();
								originalY = dddtest3.toFloat();
								originalZ = dddtest5.toFloat();
								//theApp.GenFiled(tempbbb, originalX, originalY, originalZ);
								float SetCoorX, SetCoorY, SetCoorZ;
								SetCoorX = originalX+tempX2-tempX;
								//theApp.myMessageBox(originalX);
								//theApp.myMessageBox(tempX2);
								//theApp.myMessageBox(tempX);

								//theApp.myMessageBox(SetCoorX);
								SetCoorY = originalY+tempY2-tempY;
								//theApp.myMessageBox(SetCoorY);
								SetCoorZ = originalZ+tempZ2-tempZ;//tempZ2 error
								//theApp.myMessageBox(SetCoorZ);
					//			//string dddf = temp.toStdString();
								CString XMove, YMove, ZMove, totalMove;
								XMove.Format(_T("%.2f"),SetCoorX);YMove.Format(_T("%.2f"),SetCoorY);ZMove.Format(_T("%.2f"),SetCoorZ);
					...
					//SbString diffuse,temp;
					//SbName nameFiled;
					//nameFiled = "whichChild";
					temp = "-1";
					//theApp.m_switch1 = test1Switch->getField(nameFiled);
					theApp.m_switch1->set(temp);
					theApp.m_switch2->set(temp);

					theApp.m_AlignStatus = false;
					theApp.m_HoleStatus = false;
					theApp.m_Selection->deselectAll();
					theApp.m_Selection->policy.setValue(SoSelection::SINGLE);
					theApp.m_AlignName = "";
					//int dd = theApp.xy->getValueSize();
					//theApp.myMessageBox(dd);
				}
			}
			//theApp.myMessageBox(sbStr);
		}
	...
	if (theApp.m_AlignStatus == true)
	{
		...
		if ( group->isOfType(SoFaceSet::getClassTypeId()))
		{
			...
			// 
			SoNode * BarNameNode = path->getNode(2);
			SoNode * BarNode = path->getNodeFromTail(2);
			SoNode * BodyNode = path->getNodeFromTail(1);

			SbName BarName = BarNameNode->getName();
			SbName BodyNode_Name = BodyNode->getName();
			SbName BarNode_Name = BarNode->getName();
			SbString sbStr, sbStr1, sbStr2, sbStrtemp;
			// 连接排名称
			sbStr2.fromWideChar(theApp.SbName2WChar(BarName));
			sbStr1.fromWideChar(theApp.SbName2WChar(BarNode_Name));
			//theApp.myMessageBox(sbStr1);
			int len = sbStr1.getLength();
			sbStrtemp = sbStr1.getSubString(len-8, len);
			//theApp.myMessageBox(sbStr1);
			if (sbStrtemp == "connectA" || sbStrtemp == "connectB")
			{
				// 
				if(theApp.m_AlignName == "")
				{
					theApp.m_AlignName = sbStr2;
				}
				else
				{
					if (theApp.m_AlignName == sbStr2)
					{
						theApp.myMessageBox("选择重复");
						return;
					}
				}
				//
				if (sbStrtemp == "connectA")
				{
					if (BarNode->isOfType(SoSeparator::getClassTypeId()))
					{
						//theApp.m_tempSel->addPath(path);
						//
						sbStrtemp = sbStr1 + "COLOR";
						SoNode *testColor = BarNode->getByName(sbStrtemp);
		...
		if (SelNum == 2)
		{
			SoPath * sel1 = theApp.m_Selection->getPath(0);
			SoPath * sel2 = theApp.m_Selection->getPath(1);
			sel1->ref();sel2->ref();

			int testLength = path->getLength();
				 
			SoNode * testBarNameNode1 = sel1->getNode(3);
			SoNode * testBarNameNode2 = sel2->getNode(3);

			SoNode * testBarRootNode1 = sel1->getNode(2);
			SoNode * testBarRootNode2 = sel2->getNode(2);

			SbName BarName1 = testBarNameNode1->getName();
			SbName BarName2 = testBarNameNode2->getName();
			SbName rootBarName1 = testBarRootNode1->getName();
			SbName rootBarName2 = testBarRootNode2->getName();

			SbString sbStr0test, sbStr1test, sbStr2test, sbStr3test, sbStrtemp;
			// 连接排名称
			sbStr0test.fromWideChar(theApp.SbName2WChar(BarName1));
			sbStr1test.fromWideChar(theApp.SbName2WChar(BarName2));

			sbStrtemp = sbStr0test + "HOLE_SWITCH";
			SoNode * test1Switch = testBarNameNode1->getByName(sbStrtemp);
			sbStrtemp = sbStr1test + "HOLE_SWITCH";
			SoNode * test2Switch = testBarNameNode2->getByName(sbStrtemp);

			if (test1Switch->isOfType(SoSwitch::getClassTypeId()))
			{
				SbString diffuse,temp;
				SbName nameFiled;
				nameFiled = "whichChild";
			...
			if (test2Switch->isOfType(SoSwitch::getClassTypeId()))
			{
				SbString diffuse,temp;
				SbName nameFiled;
				nameFiled = "whichChild";
				temp = "-3";
				theApp.m_switch2 = test2Switch->getField(nameFiled);
				theApp.m_switch2->set(temp);
			}
			theApp.m_AlignStatus = false;
			theApp.m_HoleStatus = true;
			SbString temp = "1 0 0";
			//theApp.xx = testColor->getField(nameFiled);
			theApp.xx->set(temp);
			theApp.yy->set(temp);
	...
	// 普通模式
	if (theApp.m_AlignStatus == false && theApp.m_HoleStatus == false)
	{
		SoSFEnum SelectMode = theApp.m_Selection->policy;
		if (SelectMode.getValue() == 1)
		{
			int SelNum = theApp.m_Selection->getNumSelected();
			...
			path->unref();
			//////////////////////////////////////////////////////////////////////////
			if (SelNum == 2 && theApp.m_HoleStatus == false)
			{
				//MessageBox(_T("begin"));
				// 是否确定对话框
				JudgementDLG judge;
		...
		if (SelectMode.getValue() == 0)
		{
			...
			if ( group->isOfType(SoFaceSet::getClassTypeId()))
			{
				...
				SbName BodyNode_Name = BodyNode->getName();
				SbName BarNode_Name = BarNode->getName();
				SbString sbStr, sbStr1, sbStr2;
				//
				sbStr2.fromWideChar(theApp.SbName2WChar(BarName));
				//theApp.myMessageBox(sbStr2);
				// 这个的值应该是Body
				sbStr.fromWideChar(theApp.SbName2WChar(BodyNode_Name));
				// 这个的值应该是IBar 名称
				sbStr1.fromWideChar(theApp.SbName2WChar(BarNode_Name));
				if (BodyNode->isOfType(SoSeparator::getClassTypeId()))
				{
					// 如果选择主体的情况
					if(sbStr == "Body")
					{
						theApp.m_BodySel = true;
						//theApp.myMessageBox(sbStr);
						// 设置主体颜色
						//SbName dd = BarNode->getName();
						SbString BarNameStr,sbStrColor;
						//sbStr1.fromWideChar(theApp.SbName2WChar(dd));
						sbStrColor.fromWideChar(_T("COLOR"));
						BarNameStr = sbStr2+sbStrColor;
						//theApp.myMessageBox(BarNameStr);
						// 设置body颜色
						SoNode *testColor = BarNode->getByName(BarNameStr);
						if (testColor->isOfType(SoMaterial::getClassTypeId()))
						{
							SbString diffuse,temp;
							SbName nameFiled;
						...
						// 设置端头颜色
						sbStr1 = sbStr2+"connectACOLOR";
						SoNode *ColorA = BarNode->getByName(sbStr1);
						if (ColorA->isOfType(SoMaterial::getClassTypeId()))
						{
							SbString diffuse,temp;
							SbName nameFiled;
							nameFiled = "diffuseColor";
							theApp.yy = ColorA->getField(nameFiled);
							/*int dddd = theApp.m_Selection->getNumSelected();
							theApp.myMessageBox(dddd);*/
							//SoPath *selectPath = theApp.m_Selection->getPath(1);
							temp = "0 1 0";
						...
						sbStr1 = sbStr2 + "connectBCOLOR";
						SoNode *ColorB = BarNode->getByName(sbStr1);
						if (ColorB->isOfType(SoMaterial::getClassTypeId()))
						{
							SbString diffuse,temp;
							SbName nameFiled;
							nameFiled = "diffuseColor";
							theApp.zz = ColorB->getField(nameFiled);
							/*int dddd = theApp.m_Selection->getNumSelected();
							theApp.myMessageBox(dddd);*/
							//SoPath *selectPath = theApp.m_Selection->getPath(1);
							temp = "0 1 0";
					...
					else if(sbStr1 == sbStr2+"connectA" || sbStr1 == sbStr2+"connectB")
					{
						//theApp.myMessageBox(sbStr1);
						if (sbStr1 == sbStr2+"connectA")
						{
							sbStr1 = sbStr2 + "connectACOLOR";
							SoNode *ColorA = BarNode->getByName(sbStr1);
							if (ColorA->isOfType(SoMaterial::getClassTypeId()))
							{
								...
								SbName nameFiled;
								nameFiled = "diffuseColor";
								theApp.xx = ColorA->getField(nameFiled);
								temp = "0 1 0";
								theApp.xx->set(temp);
							}
						}
						if (sbStr1 == sbStr2+"connectB")
						{
							sbStr1 = sbStr2 + "connectBCOLOR";
							SoNode *ColorB = BarNode->getByName(sbStr1);
							if (ColorB->isOfType(SoMaterial::getClassTypeId()))
							{
								...
								SbName nameFiled;
								nameFiled = "diffuseColor";
								theApp.xx = ColorB->getField(nameFiled);
								temp = "0 1 0";
								theApp.xx->set(temp);
							}
						}
